...
IMAGE_PATH = '/home/xnmaster/projecte/XNAPproject-grup_15/datasets_img/CACD/CACD2000'
...
BEST_MODEL_PATH = os.path.join(PATH, f'bm-rn50-CE-DA-desc{N_BLOCS_DESCONGELATS}-mig{N_NEURONES_MIG}-drp{DROPOUT_PROB}-fc1.pth')
...
wandb.init(
    # set the wandb project where this run will be logged
        ...
        name = f'RESNET34 {N_BLOCS_DESCONGELATS} descongelats - DA {DA_RATIO}% transf{transform_num}-pretrain_basic',
...
def init_resnet(grayscale, num_classes, pretrain_basic = True):
    if pretrain_basic:
...
if PRETRAIN_MODEL_PATH:
    model = init_resnet(GRAYSCALE, NUM_CLASSES,pretrain_basic=False)
    load_best_model_weights(model,PRETRAIN_MODEL_PATH)
else:
    model = init_resnet(GRAYSCALE,NUM_CLASSES)
    set_parameter_requires_grad_blocs(model, True)
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (features, targets) in enumerate(train_loader):
        ...
        # mse_loss, variance_loss = criterion(logits, targets)
        # cost = mse_loss + variance_loss

        # mean_loss, variance_loss = criterion1(logits, targets)
        # softmax_loss = criterion2(logits, targets)
        # cost = mean_loss + variance_loss + softmax_loss
