...
def metrica_propia(model, data_loader, device, logits, probas, age_group_width=5):
    total_incorrect_penalty = 0
    total_examples = 0
    ...
    with torch.no_grad():
        for features, targets in data_loader:
            ...
            age_differences = differences * age_group_width
            cost = F.cross_entropy(logits, targets)
            incorrect_penalty = torch.sum(age_differences[~correct_predictions])
            total_incorrect_penalty += incorrect_penalty
            total_examples += targets.size(0)

    metrica = (cost + total_incorrect_penalty) / total_examples
    return metrica
...
for epoch in range(num_epochs):
    ...
    with torch.set_grad_enabled(False):
        ...
        train_metrica_propia = metrica_propia(model, train_loader, DEVICE, logits, probas)
        test_metrica_propia = metrica_propia(model, test_loader, DEVICE, logits, probas)

        wandb.log({
            'epoch': epoch, 
            'train_mae': train_mae, 'train_mse': train_mse, 'train_r2': train_r2, 'train_metrica_propia':train_metrica_propia,
            'test_mae': test_mae, 'test_mse': test_mse, 'test_r2': test_r2, 'test_metrica_propia':test_metrica_propia
        })

    if test_mae < best_mae:
        best_mae, best_rmse, best_r2, best_test_metrica_propia, best_epoch = test_mae, (test_mse)**0.5, test_r2, test_metrica_propia, epoch
