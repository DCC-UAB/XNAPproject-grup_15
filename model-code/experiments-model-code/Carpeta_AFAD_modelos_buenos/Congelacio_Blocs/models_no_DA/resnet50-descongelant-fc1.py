...
BEST_MODEL_PATH = os.path.join(PATH, f'bm-rn50-desc{N_BLOCS_DESCONGELATS}-mig{N_NEURONES_MIG}-drp{DROPOUT_PROB}-fc1.pth')
...
header.append('Number of workers: %s' % NUM_WORKERS)
header.append('Number of blocs descongelats: %s' % N_BLOCS_DESCONGELATS)
header.append('Number of neurones mig: %s' % N_NEURONES_MIG)
header.append('Pretrain model path: %s' % PRETRAIN_MODEL_PATH)
header.append('Dropout probability: %s' % DROPOUT_PROB)
...
def set_parameter_requires_grad(model):
    for param in list(model.parameters())[:-N_BLOCS_DESCONGELATS]:
