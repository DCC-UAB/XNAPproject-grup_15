...
# grup 1 (0, 5)
g1_TRAIN_CSV_PATH = 'datasets/separate_ages/afad_train_grup_1.csv'
g1_TEST_CSV_PATH = 'datasets/separate_ages/afad_test_grup_1.csv'

# grup 2 (6, 11)
g2_TRAIN_CSV_PATH = 'datasets/separate_ages/afad_train_grup_2.csv'
g2_TEST_CSV_PATH = 'datasets/separate_ages/afad_test_grup_2.csv'

# grup 3 (12, 17)
g3_TRAIN_CSV_PATH = 'datasets/separate_ages/afad_train_grup_3.csv'
g3_TEST_CSV_PATH = 'datasets/separate_ages/afad_test_grup_3.csv'

# grup 4 (18, 23)
g4_TRAIN_CSV_PATH = 'datasets/separate_ages/afad_train_grup_4.csv'
g4_TEST_CSV_PATH = 'datasets/separate_ages/afad_test_grup_4.csv'

# grup 5 (24, 30)
g5_TRAIN_CSV_PATH = 'datasets/separate_ages/afad_train_grup_5.csv'
g5_TEST_CSV_PATH = 'datasets/separate_ages/afad_test_grup_5.csv'
...
DA_RATIO = 0.5
transform_num = 1
PATH = "model-code/separate_ages_models/AFAD/BM_SEP_AFAD"
BEST_MODEL_PATH = os.path.join(PATH, f'bm_AFAD_g{num_grup}.pth')
...
wandb.init(
    # set the wandb project where this run will be logged
        ...
        name = f'afad-rn50-bm-grup{num_grup}',
        # track hyperparameters and run metadata
        config={
            "learning_rate": learning_rate,
            "architecture": "MSE",
            "model": "rn50",
            "dataset": "afad-grup{num_grup}",
            "epochs": num_epochs,
            }
    )
...
class AFADDatasetAge(Dataset):
    ...
    def __init__(self, csv_path, img_dir, transform=None, augmentation_transform=None, augment_ratio=0.5):
    ...
    def __getitem__(self, index):
        ...
        if self.augmentation_transform and rand_num < self.augment_ratio:
...
augmentation_transform2 = transforms.Compose([
    ...
    transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.2),
    transforms.RandomResizedCrop(size=(120, 120), scale=(0.8, 1.0)),
    transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 2.0)),  # Applies Gaussian blur with a kernel size of 3 and sigma ranging from 0.1 to 2.0
    transforms.RandomErasing(p=0.1, scale=(0.02, 0.2), ratio=(0.3, 3.3), value=0, inplace=False),  # Randomly performs Random Erasing augmentation with a probability of 0.1
    transforms.ToTensor()
        ])
...
DA_transf = {1: augmentation_transform1, 2: augmentation_transform2}

# Create dataset instances
train_dataset = AFADDatasetAge(
    csv_path=TRAIN_CSV_PATH,
    img_dir=IMAGE_PATH,
    transform=custom_transform,
    augmentation_transform=DA_transf[transform_num],
    augment_ratio=DA_RATIO
)

test_dataset = AFADDatasetAge(
...
load_best_model_weights(model, "/home/xnmaster/projecte/XNAPproject-grup_15/model-code/experiments-model-code/Carpeta_AFAD_modelos_buenos/Congelacio_Blocs/models_DA/best_model_actual-8-blocs-desc-DA0.5-1.pth")
