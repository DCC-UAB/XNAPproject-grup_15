...
IMAGE_PATH_AFAD = '/home/xnmaster/projecte/XNAPproject-grup_15/datasets_img/AFAD/AFAD-Full'
...
BEST_MODEL_PATH = os.path.join(PATH, f'bm-rn18-DA-desc{N_BLOCS_DESCONGELATS}-mig{N_NEURONES_MIG}-drp{DROPOUT_PROB}-fc1-{learning_rate}-centered-afad-test.pth')
...
wandb.init(
    # set the wandb project where this run will be logged
        ...
        name = f'RESNET18 {N_BLOCS_DESCONGELATS} descongelats - DA {DA_RATIO}% transf{transform_num}-pretrain_basic -centered- creuat afad test',
        # track hyperparameters and run metadata
        config={
            "learning_rate": learning_rate,
            "architecture": "MSE",
            "model": f"resnet50 6 descongelats - DA {DA_RATIO}% {transform_num}-fc1",
            "dataset": "cacd (afad test)",
            "epochs": num_epochs,
...
for epoch in range(num_epochs):
    ...
    print(f'... Testing epoch [{epoch}/45] ...')
    model.eval()
