...
adad_train_path = 'datasets/afad-propi-train.csv'
adad_test_path = 'datasets/afad-propi-test.csv'
...
for i,df in enumerate([train_df, test_df]):
    ...
    # Calculate the number of rows per age
    rows_per_age = df['age'].value_counts()
    ages_to_decrease = rows_per_age[rows_per_age > 2300].index

    # Calculate the number of rows to decrease per age
    rows_to_decrease = rows_per_age[ages_to_decrease] - 2300
    ...
    for age in ages_to_decrease:
        ...
        rows_to_keep = np.random.choice(age_rows.index, random.randint(1700,2700), replace=False)
    ...
    other_ages = rows_per_age[rows_per_age <= 2300].index
    for age in other_ages:
        df_adjusted = pd.concat([df_adjusted, df[df['age'] == age]])

    # Guardar el nuevo dataset ajustado
    if i == 0:
        df_adjusted.to_csv("datasets/equal_distrib_datasets/afad-train-equi.csv", index=False)
        print("Dataset TRAIN ajustado guardado.")
    else:
        df_adjusted.to_csv("datasets/equal_distrib_datasets/afad-test-equi.csv", index=False)
